  ...
  return (
    <nav className="fixed top-3 sm:top-4 left-1/2 -translate-x-1/2 w-[94%] sm:w-[92%] max-w-6xl z-40 rounded-2xl sm:rounded-full glassmorphism px-3 sm:px-6 py-2.5 sm:py-3.5 flex items-center justify-between gap-2 border border-slate-800/80 shadow-2xl">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          type="button"
          className="md:hidden p-2 rounded-full bg-slate-800/80 border border-slate-700 text-white cursor-pointer"
          ...
          <div className="flex flex-col">
            {NAV_LINKS.map(link => (
              <a
                ...
                href={link.href}
                onClick={closeMenu}
                className="px-3 py-3 text-sm font-medium text-slate-200 hover:text-brand-neon hover:bg-slate-800/70 rounded-xl transition"
              >
                {link.label}
              </a>
            ))}
